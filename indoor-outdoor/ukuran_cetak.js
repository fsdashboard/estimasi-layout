// ================== KONFIGURASI UMUM ==================
const PANEL_PENALTY = 2.0;
const SUGGESTION_MIN_PERCENT = 2;
const SUGGESTION_MIN_AREA = 0.2;
...
function minimalNaikTerdekat(nilai, minimalList) {
  return minimalList.find(v => v >= nilai) ?? nilai;
}

function findRollForSizes(maxSize, rolls) {
  const candidates = rolls.filter(r => r.maksimal >= maxSize);
  return candidates.length ? candidates.sort((a, b) => a.width - b.width)[0] : null;
}
...
function isAtMaksimal(value, rolls) {
  return rolls.some(r => Math.abs(r.maksimal - value) < 1e-6);
}

function upperStandard(nilai, minimalList) {
  return minimalList.find(v => v >= nilai) ?? null;
}
...
function updateBahanWarning() {
  ...
  const info = bahanData[el.value];
  if (info?.seamNotRecommended) {
    warn.classList.remove("hidden");
    warn.innerText = info.seamWarning;
  } else {
...
// ================== RESET ==================
function resetForm() {
  const p = document.getElementById("panjang");
  const l = document.getElementById("lebar");
  const b = document.getElementById("bahan");
  ...
  if (b) {
    b.value = "";
    if (window.$) $('#bahan').val(null).trigger('change');
  }

  if (h) {
    h.classList.add("hidden");
    ["ukuranCetak", "jumlahPanel", "ukuranPanel", "keterangan"].forEach(id => {
