...
function hitungSpiral() {
  ...
  if (!halaman || !cetak || !panjang || !lebar || !bahanCover || !sisiCover || !mediaCover || !bahanIsi || !sisiIsi || !mediaIsi) {
    showAlertModal('Lengkapi semua input Spiral.');
    return;
  }
  ...
  let acuanIsi = mediaIsi === "A3+" ? [47.5, 31.5] : mediaIsi === "A3" ? [42, 29.7] : [29.7, 21];
  ...
  if (totalCover <= 0) {
    showAlertModal('Ukuran jadi melebihi media cetak Cover.');
    return;
  }

  const fit1Isi = Math.floor(acuanIsi[0] / panjang) * Math.floor(acuanIsi[1] / lebar);
  const fit2Isi = Math.floor(acuanIsi[0] / lebar) * Math.floor(acuanIsi[1] / panjang);
  const totalIsi = Math.max(fit1Isi, fit2Isi);

  if (totalIsi <= 0) {
    showAlertModal('Ukuran jadi melebihi media cetak Isi.');
    return;
  }

  document.getElementById('spiral-total-cover').value = `${totalCover} pcs`;
  document.getElementById('spiral-total-isi').value = `${totalIsi} pcs`;

  const lembarCover = Math.ceil((2 * cetak) / totalCover);

  let sisaHalaman = halaman - (sisiCover === "1" ? 2 : 4);
  let lembarIsi = sisiIsi === "1"
    ? Math.ceil((sisaHalaman * cetak) / totalIsi)
    : Math.ceil(((sisaHalaman / 2) * cetak) / totalIsi);
  ...
  const tebalIsi = (sisiIsi === "1" ? sisaHalaman : sisaHalaman / 2) * ketebalanBahan[bahanIsi];
